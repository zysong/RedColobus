...
pdf("../Manuscript/figures/A1B2plot%01d.pdf", onefile=FALSE)
#plot time series
plot.m<-matplot(N.m.mat, ylim = c(0, 100), xlab="Time", ylab="Male group size", type = "l", main="FBD+LVI", cex.lab = 1.5);
plot.f<-matplot(N.f.mat, ylim = c(0, 250), xlab="Time", ylab="Female group size", type = "l", main="FBD+LVI", cex.lab = 1.5);
plot.g<-matplot(N.f.mat+N.m.mat, ylim = c(0, 300), xlab="Time", ylab="Group size", type = "l", main="FBD+LVI", cex.lab = 1.5);
plot.r<-matplot(N.m.mat/N.f.mat, ylim = c(0, 1), xlab="Time", ylab = "Sex ratio", type = "l", main="FBD+LVI", cex.lab = 1.5);

#plot variances
plot.var<-boxplot(df.var, ylim=c(0, 3000), ylab="Variance", main="FBD+LVI", cex.lab = 1.5);
plot.cov<-boxplot(df.cov, ylim=c(0, 1.1), ylab="Coefficient of variation", main="FBD+LVI", cex.lab = 1.5);
